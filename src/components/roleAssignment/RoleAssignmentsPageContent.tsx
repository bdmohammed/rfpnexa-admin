...
import { useAssignUserRoles } from '@/features/auth/api/mutations';
import { useRoles } from '@/features/rbac/api/queries';
import { useCreateAssignment } from '@/features/rbac/api/mutations';
...
interface RoleAssignmentActionParams {
  data: RowItem;
}
...
export function RoleAssignmentModal({ user, onClose }: RoleAssignmentModalProps) {
  const [selectedRoleId, setSelectedRoleId] = useState(user.roleId ?? '');

  const { data: roles, isLoading: isRolesLoading } = useRoles(true);

  const assignRoleMutation = useCreateAssignment();
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      // Mutation hook should handle/display the API error.
    }
  };
  ...
  return (
    ...
    >
      ...
      >
        {/* Header */}
        <div className="mb-5">
          <h2 id="role-assignment-title" className="text-lg font-semibold text-text">
            Assign Role
          </h2>

          <p className="mt-1 text-sm text-text-light">Assign a role to this admin user.</p>
        </div>
        ...
        <div className="mb-5 rounded-lg border border-border bg-background px-4 py-3">
          ...
          <p className="mt-0.5 truncate text-xs text-text-light">{user.email}</p>
        </div>

        {/* Role */}
        <div>
          <label htmlFor="role-assignment" className="mb-1.5 block text-sm font-medium text-text">
            Role
          ...
          <select
            ...
            className="w-full rounded-lg border border-border bg-surface px-3 py-2.5 text-sm text-text outline-none transition focus:border-primary disabled:cursor-not-allowed disabled:opacity-60"
          >
        ...
        {user.roleName && (
          <p className="mt-2 text-xs text-text-light">
            Current role: <span className="font-medium text-text">{user.roleName}</span>
          </p>
        )}
        ...
        <div className="mt-6 flex justify-end gap-2">
          <button
            ...
            className="rounded-lg border border-border px-4 py-2 text-sm font-medium text-text transition hover:bg-background disabled:cursor-not-allowed disabled:opacity-50"
          >
            Cancel
          </button>
          ...
          <button
            ...
            disabled={!selectedRoleId || isRolesLoading || assignRoleMutation.isPending}
...
export function RoleAssignmentsPageContent() {
  const gridRef = useRef<AgGridReact>(null as any);
  const currentTheme = useThemeStore((state) => state.theme);
  // Top-level main tabs: stats | list

  // Data State
  const { data: userLists, isLoading } = useListUsers({
    accountType: 'admin',
  });

  const [selectedUser, setSelectedUser] = useState<RowItem | null>(null);
  const [isRoleModalOpen, setIsRoleModalOpen] = useState(false);

  const handleOpenRoleModal = (user: RowItem) => {
  ...
  const RoleAssignmentActionRenderer = (params: ICellRendererParams<RowItem>) => {
    if (!params.data) {
      return null;
    }
    ...
    return (
      <button
        type="button"
        onClick={() => handleOpenRoleModal(params.data!)}
        className="rounded-md border border-border px-3 py-1.5 text-xs font-medium text-text transition hover:bg-background"
      >
        Assign Role
      </button>
    );
  ...
  const desktopDefaultCols: (ColDef<RowItem> | ColGroupDef<RowItem>)[] = [
    ...
    },
    {
      ...
      enablePivot: false,
      pinned: 'right',
      cellRenderer: RoleAssignmentActionRenderer,
    },
  ];
  ...
  return (
    <div className="space-y-6">
      ...
      {isRoleModalOpen && selectedUser && (
        <RoleAssignmentModal user={selectedUser} onClose={handleCloseRoleModal} />
      )}
    </div>
  );
}
